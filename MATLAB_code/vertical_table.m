...
comparisonTable = table(samples.("TargetVz(m/s)"), samples.("AvgPower(W)"), P_fit_prop, P_fit_apm, ...
    'VariableNames', {'TargetVz_mps','MeasuredPower_W','ProposedFit_W','APMFit_W'});
disp('=== Comparison Table: Measured vs Proposed vs APM ===');
disp(comparisonTable);

%% === Step 5: Merge with Reference Parameters (mav.param / Excel) ===
% Example: load your reference constants from Excel
% Ensure your Excel file has columns matching parameter names
% e.g., W, n, δ, s, C_T, k1, k2, etc.% === Reference Parameters (from mav.param / datasheet) ===
refParams = {'W','n','S_fp_perp','rho','A','δ','s','C_T','k', ...
    'k1','k2','c2','c4','mg','c5'};

% Example values — replace with your actual UAV constants
refValues = [16, 4, 0.02, 1.225, 0.126, 0.011, 0.045, 0.0012, 0.11, ...
    0.85, 0.52, 0.3, 0.03, 17, 0.028];

% Build reference table
refData = table(refParams', refValues', 'VariableNames', {'Parameter','ReferenceValue'});

disp('=== Reference Parameters from mav.param / datasheet ===');
disp(refData);



% Merge into one summary table
summaryAll = [resultsTableProp; resultsTableAPM; refData];
writetable(summaryAll,'vertical_ascent_parameter_comparison.xlsx');

% Save comparison table as well
writetable(comparisonTable,'vertical_ascent_power_comparison.xlsx');
