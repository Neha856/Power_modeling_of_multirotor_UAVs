...
figure;
scatter(samples.("TargetVx(m/s)"), samples.("AvgPower(W)"), 60, 'b','filled'); hold on;
plot(samples.("TargetVx(m/s)"), P_fit_prop, 'r--','LineWidth',2);
plot(samples.("TargetVx(m/s)"), P_fit_apm, 'm-.','LineWidth',2);   % <-- corrected
xlabel('Horizontal Speed (m/s)');
ylabel('Average Power (W)');
title('Horizontal Flight Power Curve (0–10 m/s, integer steps)');
legend('Measured Avg Samples','Proposed Horizontal Model','APM Aerodynamic Model');
