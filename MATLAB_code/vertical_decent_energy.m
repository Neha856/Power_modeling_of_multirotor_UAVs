% === Load Filtered CSV Data (Vertical Descent) ===
data = readtable('vertical_descent_filtered_20tol_1Hz.csv','VariableNamingRule','preserve');

TargetVz = abs(data.("TargetVz(m/s)"));   % absolute values for descent
Power    = data.("Power(W)");
...
%% Step 3: Proposed Vertical Descent Model
...
%% Step 4: APM Aerodynamic Descent Model
...
fprintf('Average Relative Difference (Proposed Descent Energy Model): %.2f %%\n', relDiffProp);
fprintf('Average Relative Difference (APM Descent Energy Model): %.2f %%\n', relDiffAPM);
...
xlabel('Vertical Descent Speed (m/s)');
ylabel('Energy per Distance (J/m)');
title('Vertical Descent Energy Efficiency vs Speed');
legend('Measured Avg Energy/Distance','Proposed Model Based','APM Model Based');
xlim([0 3.5]); ylim([0 800]); xticks(0:0.5:3.5); grid on;
