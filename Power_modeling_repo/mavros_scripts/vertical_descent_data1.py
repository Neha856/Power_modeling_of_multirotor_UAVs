...
if __name__ == '__main__':
    try:
        print("🚀 Initializing continuous vertical descent experiment...")
        set_flight_mode('GUIDED')
        arm_drone()
        takeoff_drone(160)

        # Wait until UAV actually reaches ~110 m
        while current_alt < 159.0:
            rospy.sleep(1)
        print("✅ Reached 160 m, hovering...")
        rospy.sleep(5)  # hover stabilize

        filename = '/home/neha/ardupilot/ArduCopter/vertical_descent_data2.csv'
        with open(filename, 'w') as f:
            ...
            total_drop = 160.0 - 1.5
            band_height = total_drop / len(descent_speeds)  # altitude allocated per speed step

            current_alt = 160.0
            for spd in descent_speeds:
                target_alt = max(1.5, current_alt - band_height)
        ...
        print(f"🎯 Continuous descent data logged successfully. CSV saved at: {filename}")
