...
if __name__ == '__main__':
    try:
        print("🚀 Horizontal flight experiment (0–15 m/s in one take)")
        set_flight_mode('GUIDED')
        arm_drone()
        takeoff_drone(20)
        rospy.sleep(15)

        filename = '/home/neha/ardupilot/ArduCopter/horizontal_flight_data_full.csv'
        with open(filename, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(["Timestamp","Velocity(m/s)","Voltage(V)","Current(A)","Power(W)","TargetSpeed(m/s)"])
            for spd in range(0, 11, 1):   # sweep 0–15 m/s
