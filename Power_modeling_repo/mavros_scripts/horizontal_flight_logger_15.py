...
from mavros_msgs.msg import ParamValue   # for WPNAV_SPEED
...
# === Init ROS node ===
rospy.init_node('horizontal_flight_logger_15')
...
# Optional: set WPNAV_SPEED to 1500 cm/s (15 m/s) so AUTO missions allow higher speed
try:
    rospy.wait_for_service('/mavros/param/set', timeout=5)
    param_set_client = rospy.ServiceProxy('/mavros/param/set', ParamSet)
    val = ParamValue(integer=0, real=1500.0)  # 1500 cm/s = 15 m/s
    param_set_client("WPNAV_SPEED", val)
    print("✅ WPNAV_SPEED set to 15 m/s")
except Exception as e:
    print("⚠️ Could not set WPNAV_SPEED:", e)

# === Main sequence ===
if __name__ == '__main__':
    try:
        print("🚀 Horizontal flight experiment (Hover + 1–15 m/s sweep)")
        set_flight_mode('GUIDED')
        arm_drone()
        takeoff_drone(20)
        rospy.sleep(20)  # settle after takeoff

        filename = '/home/neha/ardupilot/ArduCopter/horizontal_flight_data_15.csv'
        with open(filename, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(["Timestamp","Velocity(m/s)","Voltage(V)","Current(A)","Power(W)","TargetSpeed(m/s)"])

            # === Hover segment at 0 m/s ===
            print("🛑 Hover at 0 m/s for 20s")
            start_time = time.time()
            while time.time() - start_time < 20:
                move_with_velocity(0, 0, 0)
                ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                power = voltage * current
                writer.writerow([ts, round(velocity,3), round(voltage,3), round(current,3), round(power,3), 0])
                rospy.sleep(0.1)  # 10 Hz logging

            # === Forward flight sweep 1–15 m/s ===
            for spd in range(1, 16, 1):
                print(f"➡️ Speed step {spd} m/s")
                start_time = time.time()
                while time.time() - start_time < 20:  # hold each speed for 20s
        ...
        print(f"🎯 Experiment complete. CSV saved at: {filename}")
