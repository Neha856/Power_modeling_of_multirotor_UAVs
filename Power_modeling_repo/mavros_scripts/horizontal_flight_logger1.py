...
if __name__ == '__main__':
    try:
        print("🚀 Horizontal flight experiment (Part 1: 0–7 m/s)")
        set_flight_mode('GUIDED')
        arm_drone()
        takeoff_drone(20)
        rospy.sleep(15)

        filename1 = '/home/neha/ardupilot/ArduCopter/horizontal_flight_data_part1.csv'
        with open(filename1, 'w') as f1:
            writer = csv.writer(f1)
            writer.writerow(["Timestamp","Velocity(m/s)","Voltage(V)","Current(A)","Power(W)","TargetSpeed(m/s)"])
            for spd in range(0, 8, 1):
        ...
        print("🎯 Part 1 complete. Restart SITL before Part 2.")
